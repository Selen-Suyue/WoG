...
class VLAMetrics:
    def __init__(
        ...
        self.state = {
            "loss_raw": deque(maxlen=grad_accumulation_steps),
            "loss": deque(maxlen=window_size),
            "act_loss": deque(maxlen=window_size),
            "obs_loss": deque(maxlen=window_size),
            "step_time": deque(maxlen=window_size),
            "lr": [],
        }
        ...
        **kwargs,
        ...
        for key, value in kwargs.items():
            if key == "loss":
                ...
                self.state[f"{key}"].append(loss_val)
            else:
                self.state[key].append(value.detach())
    ...
    def push(self) -> str:
        ...
        loss_raw = torch.stack(list(self.state["loss_raw"])).mean().item()
        loss = torch.stack(list(self.state["loss"])).mean().item()
        act_loss = torch.stack(list(self.state["act_loss"])).mean().item()
        obs_loss = torch.stack(list(self.state["obs_loss"])).mean().item()
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Loss": loss,
                f"{prefix}/Act Loss": act_loss,
                f"{prefix}/Obs Loss": obs_loss,
                f"{prefix}/Loss (Raw)": loss_raw,
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
            },
