...
def chunk_act_obs(
    ...
    future_obs_column_indices = [
        0,
        future_action_window_size // 4,
        future_action_window_size // 2,
        3 * future_action_window_size // 4,
        window_size + future_action_window_size - 1,
    ]

    future_obs_indices = tf.gather(floored_action_chunk_indices, future_obs_column_indices, axis=1)

    traj["observation"] = tf.nest.map_structure(lambda x: tf.gather(x, future_obs_indices), traj["observation"])
    unfloored_future_obs_indices = tf.gather(action_chunk_indices, future_obs_column_indices, axis=1)
    traj["observation"]["pad_mask"] = unfloored_future_obs_indices >= 0
    ...
    floored_action_chunk_indices_for_masking = tf.minimum(
        tf.maximum(action_chunk_indices, 0), goal_timestep[:, None] + 5
    )
