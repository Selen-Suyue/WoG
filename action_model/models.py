...
#################################################################################
#                      Embedding Layers for Actions and obs                     #
#################################################################################
...
class DiTBlock(nn.Module):
    """
    A DiT block with perceptual-attention and cognitive-attention conditioning.
    """
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, use_per_attn=False, **block_kwargs):
        ...
        if self.use_per_attn:
            ...
            nn.init.constant_(self.per_attn.in_proj_weight, 0.0)
            nn.init.constant_(self.per_attn.in_proj_bias, 0.0)
            nn.init.constant_(self.per_attn.out_proj.weight, 0.0)
            nn.init.constant_(self.per_attn.out_proj.bias, 0.0)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.use_per_attn = use_per_attn
        self.per_token_size = token_size
        ...
        if self.use_per_attn:
            self.per_token_embedder = LabelEmbedder(
                in_size=inj_channels,
                hidden_size=hidden_size,
                dropout_prob=obs_dropout_prob,
                conditions_shape=(1, 1, inj_channels),
            )
        ...
        self.positional_embedding = nn.Parameter(scale * torch.randn(future_action_window_size + 2, hidden_size))
    ...
    def initialize_weights(self):
        ...
        if self.use_per_attn:
            if self.obs_dropout_prob > 0:
                nn.init.normal_(self.per_token_embedder.uncondition, std=0.02)
    ...
    def forward(self, x, t, z, per_token=None):
        ...
        if self.use_per_attn:
            if per_token is not None:
                per_token = self.per_token_embedder(per_token, self.training)
            else:
                uncond = self.per_token_embedder.uncondition.to(dtype=x.dtype)
                uncond_batch = uncond.unsqueeze(0).expand(x.shape[0], -1, -1)
                per_token = self.per_token_embedder.linear(uncond_batch)
